...
import { PlusIcon, EditIcon, DeleteIcon, LoadingSpinner, SparklesIcon, CheckCircleIcon, DocumentTextIcon } from './icons';
...
const PlayerForm: React.FC<PlayerFormProps> = ({ onSave, tournament, playerToEdit }) => {
    ...
    const handleStatChange = (field: keyof PlayerStats, value: string) => {
        setStats(prev => {
            const newStats = prev ? { ...prev } : { matchesPlayed: 0, totalScore: 0, totalWickets: 0 };
            newStats[field] = parseInt(value, 10) || 0;
            return newStats;
        });
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-2 text-right">
                <button type="submit" disabled={!stats || isGenerating} className="inline-flex justify-center items-center gap-2 bg-brand-primary hover:bg-brand-primary/80 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
